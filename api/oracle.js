...
module.exports = async function handler(req, res) {
  ...
  var maxAge  = reqDate === today ? 120000 : 3600000; // 2 min today, 1hr past
  ...
  try {
    ...
    (raw.matches || []).forEach(function(m, idx) {
      ...
      // Heuristic: if kickoff was >115 min ago and still showing live/paused,
      // the API hasn't updated yet — treat as likely finished (pending real result)
      if (isLive && m.utcDate) {
        var koMs  = new Date(m.utcDate).getTime();
        var minsElapsed = (Date.now() - koMs) / 60000;
        if (minsElapsed > 115) {
          isLive     = false;
          isFinished = true; // will show as finished, score pending API update
        }
      }
