// api/predict.js — ScoutAI
// football-data.org — shows ALL competitions on free tier
// Free tier includes: Premier League, Championship, Bundesliga, Serie A,
// La Liga, Ligue 1, Champions League, Europa League, Eredivisie,
// Primeira Liga, Brasileiro, European Championship, World Cup + qualifiers

var cache = { date: null, data: null, fetched: 0 };

module.exports = async function handler(req, res) {
  res.setHeader('Access-Control-Allow-Origin', '*');
  if (req.method === 'OPTIONS') { res.status(200).end(); return; }

  var now   = new Date();
  var today = now.toISOString().slice(0, 10);

  var cacheAge = Date.now() - (cache.fetched || 0);
  if (cache.date === today && cache.data && cacheAge < 900000) {
    return res.status(200).json(buildResponse(cache.data, now));
  }

  var apiKey = process.env.FOOTBALL_DATA_KEY;
  if (!apiKey) {
    return res.status(500).json({ error: 'FOOTBALL_DATA_KEY not set in Vercel Environment Variables.' });
  }

  // Look 3 days ahead to always find fixtures
  var d3 = new Date(now); d3.setDate(d3.getDate() + 3);
  var d3str = d3.toISOString().slice(0, 10);

  var allMatches = [];

  try {
    // Fetch ALL matches (no competition filter) — API returns whatever your tier allows
    var r = await fetch(
      'https://api.football-data.org/v4/matches?dateFrom=' + today + '&dateTo=' + d3str,
      { headers: { 'X-Auth-Token': apiKey } }
    );

    if (!r.ok) {
      var errData = await r.json().catch(function(){ return {}; });
      return res.status(502).json({
        error: 'football-data.org error (' + r.status + '): ' + (errData.message || 'Check your API key.')
      });
    }

    var data = await r.json();
    var matches = data.matches || [];

    matches.forEach(function(m) {
      // Include not-started and live matches
      var ok = ['SCHEDULED','TIMED','IN_PLAY','PAUSED'].indexOf(m.status) !== -1;
      if (!ok) return;

      var home = m.homeTeam && (m.homeTeam.shortName || m.homeTeam.name);
      var away = m.awayTeam && (m.awayTeam.shortName || m.awayTeam.name);
      if (!home || !away || home === 'null' || away === 'null') return;

      var league     = (m.competition && (m.competition.name)) || 'Unknown';
      var matchDate  = m.utcDate ? m.utcDate.slice(0, 10) : today;
      var isLive     = m.status === 'IN_PLAY' || m.status === 'PAUSED';
      var daysAhead  = Math.round((new Date(matchDate) - new Date(today)) / 86400000);

      allMatches.push({
        home:        home,
        away:        away,
        league:      league,
        kickoff_iso: m.utcDate,
        is_live:     isLive,
        days_ahead:  daysAhead,
        status:      m.status
      });
    });

  } catch(err) {
    return res.status(502).json({ error: 'Fetch failed: ' + err.message });
  }

  if (!allMatches.length) {
    return res.status(200).json({
      predictions: [], edge: [], source: 'live',
      message: 'No fixtures found in the next 3 days. This may be an international break week.'
    });
  }

  // ── Prediction engine ─────────────────────────────────────
  var seedBase = parseInt(today.replace(/-/g,''), 10);
  var si = 0;
  function rnd(){ var x=Math.sin(seedBase+(++si)*127773+49297)*43758.5453; return x-Math.floor(x); }
  function pick(arr){ return arr[Math.floor(rnd()*arr.length)]; }
  function range(a,b){ return Math.floor(rnd()*(b-a+1))+a; }

  var predTypes  = ['Home Win','Home Win','Home Win','Away Win','Away Win','Draw','Home Win or Draw','Both Teams to Score'];
  var goalMkts   = ['Over 2.5 Goals','Under 2.5 Goals','Over 1.5 Goals','BTTS Yes','BTTS No','Over 2.5 Goals'];
  var edgeLvls   = ['none','none','low','medium','medium','high','elite'];
  var posFacts   = ['Strong home record this season','Title race pressure drives motivation','Revenge fixture after earlier defeat','Derby atmosphere expected','Unbeaten in last 6 at home','Star striker in top form','5-game winning run adds momentum','Manager\'s 100th game in charge'];
  var negFacts   = ['Key midfielder serving suspension','Top scorer rated doubtful','Heavy fixture congestion','Four players on the injury list','Starting centre-back ruled out','Poor away form this season'];
  var verdicts   = ['Form and motivation firmly favour the home side.','Tactical discipline likely to be decisive.','Goals expected as both teams push forward.','Narrative pressure creates an unpredictable edge.','Away side\'s form makes this far closer than it looks.','High stakes on both ends add significant weight.','Revenge angle adds extra fire to the visiting side.','Home support expected to be decisive tonight.'];

  var predictions = allMatches.map(function(m, idx) {
    si = (seedBase % 999) + idx * 31 + m.home.length * 7;
    m.prediction       = pick(predTypes);
    m.confidence       = range(50, 88);
    m.goals_prediction = rnd() > 0.15 ? pick(goalMkts) : null;
    m.goals_confidence = m.goals_prediction ? range(52, 83) : null;
    var elvl   = pick(edgeLvls);
    var escore = elvl==='none'?range(5,25):elvl==='low'?range(30,48):elvl==='medium'?range(50,68):elvl==='high'?range(70,84):range(85,97);
    var nf     = (elvl==='none'||elvl==='low') ? 1 : 2;
    var facts  = [];
    for(var f=0;f<nf;f++) facts.push({label:f===0?pick(posFacts):pick(negFacts),type:f===0?'positive':'negative'});
    m._edge = {index:idx, edge_score:escore, edge_level:elvl, factors:facts, verdict:pick(verdicts)};
    return m;
  });

  var edgeData = predictions.map(function(m){ var e=m._edge; delete m._edge; return e; });
  var raw = { predictions:predictions, edge:edgeData, source:'live', fetched_at:now.toISOString() };
  cache = { date:today, data:raw, fetched:Date.now() };

  return res.status(200).json(buildResponse(raw, now));
};

function buildResponse(raw, now) {
  var preds = raw.predictions || [];
  var sorted = preds.slice().sort(function(a, b) {
    if (a.is_live && !b.is_live) return -1;
    if (!a.is_live && b.is_live) return 1;
    return new Date(a.kickoff_iso) - new Date(b.kickoff_iso);
  });
  var edgeMap = {};
  (raw.edge||[]).forEach(function(e, i){ edgeMap[i] = e; });
  var edge = sorted.map(function(m, i) {
    var orig = (raw.predictions||[]).indexOf(m);
    var e = edgeMap[orig] || {index:i, edge_score:20, edge_level:'none', factors:[], verdict:''};
    return Object.assign({}, e, {index:i});
  });
  return { predictions:sorted, edge:edge, source:raw.source, fetched_at:raw.fetched_at };
}
