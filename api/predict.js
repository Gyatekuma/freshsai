...
// Shows: upcoming + live matches. Falls forward to next 24hrs if today is sparse.
...
module.exports = async function handler(req, res) {
  ...
  if (cache.date === today && cache.data && cacheAge < 900000) { // 15-min cache
  ...
  // Fetch today + tomorrow to always have fixtures
  ...
  try {
    var r = await fetch(
      'https://api.football-data.org/v4/matches?dateFrom=' + today + '&dateTo=' + tomorrowStr,
    ...
    if (!r.ok) {
      ...
      return res.status(502).json({
        error: 'football-data.org error (' + r.status + '): ' + (err.message || 'Check your API key.')
      });
    }
    ...
    matches.forEach(function(m) {
      ...
      // Include: not yet started, currently live, or paused
      ...
      allMatches.push({
        home:        home,
        away:        away,
        league:      COMP_NAMES[compCode],
        kickoff_iso: m.utcDate,
        is_live:     isLive,
        is_tomorrow: matchDate !== today,
        status:      m.status
      });
    });
  ...
  if (!allMatches.length) {
    return res.status(200).json({
      ...
      message: 'No fixtures found in the top leagues for today or tomorrow. The major leagues may be on an international break.'
    });
  }
  ...
  var verdicts    = ['Form and motivation firmly favour the home side.','Tactical discipline likely to be decisive.','Goals expected as both teams push forward.','Narrative pressure creates an unpredictable edge.','Away side\'s current form makes this far closer than it looks.','High stakes on both ends add significant weight.','Revenge angle adds extra fire to the visiting side.','Home support expected to be a decisive factor.'];
  ...
  var predictions = allMatches.map(function(m, idx) {
    ...
    m.goals_prediction  = rnd()>0.15 ? pick(goalMarkets) : null;
...
function buildResponse(raw, now) {
  ...
  var origIdxMap = {};
  preds.forEach(function(m,i){ origIdxMap[i]=i; });
  var edge = sorted.map(function(m,i) {
    var orig = preds.indexOf(m);
