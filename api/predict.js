...
function norm(s){
  var r = (s||'').toLowerCase();
  // Explicit accent/special char map (reliable cross-platform)
  r = r.replace(/[áàâãä]/g,'a').replace(/[éèêë]/g,'e').replace(/[íìîï]/g,'i')
       .replace(/[óòôõö]/g,'o').replace(/[úùûü]/g,'u').replace(/[ñ]/g,'n')
       .replace(/[ç]/g,'c').replace(/[ý]/g,'y').replace(/[ß]/g,'ss');
  // Remove city state suffixes (Brazilian teams)
  r = r.replace(/-[a-z]{2}/g,'');
  // Remove common noise words
  r = r.replace(/(fc|sc|ac|cf|fk|sk|if|bk|club|cd|afc|rj|sp|mg|pr)/g,'');
