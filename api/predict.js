...
async function fallbackFootballData(reqDate, now, res) {
  ...
  try {
    ...
    if (!target) return res.status(200).json({ predictions:[], edge:[], date:reqDate, requested:reqDate,
      message:'No upcoming fixtures found. BetMiner quota resets tomorrow.', fallback:true });
    var preds = byDate[target];
    return res.status(200).json({ predictions:preds, edge:[], date:target, requested:reqDate,
      fetched_at:now.toISOString(), fallback:true,
      notice:'⚠️ Limited data — BetMiner quota resets midnight UTC. Showing fixtures only, no predictions.' });
  } catch(err) {
    return res.status(503).json({ error: 'BetMiner quota reached. Predictions will refresh tomorrow.' });
  }
}
